...
int Pin2dmdInit() {
  ...
  // Now look through the list that we just populated. We are trying to see if
  // any of them match our device.
  int i;
  ...
  ret = libusb_get_string_descriptor_ascii(MyLibusbDeviceHandle, desc.iProduct,
                                           product, 256);

  if (libusb_claim_interface(MyLibusbDeviceHandle, 0) <
      0)  // claims the interface with the Operating System
  {
...
void Pin2dmdRender(UINT16 width, UINT16 height, UINT8* Buffer, int bitDepth) {
  if ((width == 256 && height == 64 && Pin2dmdHD) ||
    ...
    // The OutputBuffer to be sent consists of a 4 byte header and a number of
    // chunks of 512 bytes.
    libusb_bulk_transfer(MyLibusbDeviceHandle, EP_OUT, OutputBuffer,
                         (chunksOf512Bytes * 512) + 4, NULL, 1000);
  }
}
