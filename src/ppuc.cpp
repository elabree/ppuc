...
static struct cag_option options[] = {
    ...
    {.identifier = 'r',
     ...
     .value_name = "VALUE",
     .description =
         "Path to ROM file (optional, overwrites setting in config file)"},
    {.identifier = 's',
     .access_letters = "s",
     .access_name = "serial",
     .value_name = "VALUE",
     .description =
         "Serial device (optional, overwrites setting in config file)"},
    {.identifier = 'n',
     .access_letters = "n",
    ...
    {.identifier = 't',
     ...
     .value_name = "VALUE",
     .description =
         "Serum timeout in milliseconds to ignore unknown frames (optional)"},
    {.identifier = 'p',
     .access_letters = "p",
...
void PINMAMECALLBACK OnStateUpdated(int state, const void* p_userData) {
  ...
  if (!state) {
    ...
    mechConfig.type = PINMAME_MECH_FLAGS_NONLINEAR |
                      PINMAME_MECH_FLAGS_REVERSE | PINMAME_MECH_FLAGS_ONESOL;
...
void PINMAMECALLBACK OnDisplayAvailable(int index, int displayCount,
  ...
  if (opt_debug) {
    printf(
        ...
        "depth=%d, length=%d\n",
        index, displayCount, p_displayLayout->type, p_displayLayout->top,
        p_displayLayout->left, p_displayLayout->width, p_displayLayout->height,
...
void PINMAMECALLBACK OnDisplayUpdated(int index, void* p_displayData,
  ...
  if (opt_debug) {
    printf(
        ...
        index, p_displayLayout->type, p_displayLayout->top,
        p_displayLayout->left, p_displayLayout->width, p_displayLayout->height,
        p_displayLayout->depth, p_displayLayout->length);
  }
...
int PINMAMECALLBACK OnAudioAvailable(PinmameAudioInfo* p_audioInfo,
  ...
  if (opt_debug) {
    printf(
        ...
        "bufferSize=%d\n",
        p_audioInfo->format, p_audioInfo->channels, p_audioInfo->sampleRate,
        p_audioInfo->framesPerSecond, p_audioInfo->samplesPerFrame,
  ...
  for (int index = 0; index < MAX_AUDIO_BUFFERS; index++) {
    int bufferSize =
        p_audioInfo->samplesPerFrame * _audioChannels * sizeof(int16_t);
    ...
    alBufferData(_audioBuffers[index],
                 ...
                 p_buffer, bufferSize, _audioSampleRate);
  }
...
int PINMAMECALLBACK OnAudioUpdated(void* p_buffer, int samples,
  ...
  while (buffersProcessed > 0) {
    ...
    if (_audioQueue.size() > 0) {
      ...
      alBufferData(buffer,
                   _audioChannels == 2 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
                   p_destBuffer, bufferSize, _audioSampleRate);
...
void PINMAMECALLBACK OnMechAvailable(int mechNo, PinmameMechInfo* p_mechInfo,
  ...
  if (opt_debug) {
    printf(
        ...
        "speed=%d\n",
        mechNo, p_mechInfo->type, p_mechInfo->length, p_mechInfo->steps,
        p_mechInfo->pos, p_mechInfo->speed);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  snprintf((char*)config.vpmPath, PINMAME_MAX_PATH, "%s%s\\pinmame\\",
