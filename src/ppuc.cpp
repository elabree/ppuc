...
void CALLBACK
OnStateUpdated(int state, const void* p_userData) {
    if (opt_debug) {
...
void CALLBACK
OnLogMessage(PINMAME_LOG_LEVEL logLevel, const char* format, va_list args, const void* p_userData) {
    char buffer[1024];
...
void CALLBACK
OnDisplayAvailable(int index, int displayCount, PinmameDisplayLayout* p_displayLayout, const void* p_userData) {
    if (opt_debug) {
        printf("OnDisplayAvailable(): index=%d, displayCount=%d, type=%d, top=%d, left=%d, width=%d, height=%d, "
               "depth=%d, length=%d\n",
...
void CALLBACK
OnDisplayUpdated(int index, void* p_displayData, PinmameDisplayLayout* p_displayLayout, const void* p_userData) {
    ...
    while (true) {
        ...
        while (pin2dmd_frame_buffer_position != dmd_frame_buffer_position) {
            dmdConvertToFrame(dmd_frame_buffer_width[pin2dmd_frame_buffer_position],
                              ...
                              dmd_frame_buffer[pin2dmd_frame_buffer_position], dmd_planes_buffer,
                              dmd_frame_buffer_depth[pin2dmd_frame_buffer_position]);
            Pin2dmdRender(dmd_frame_buffer_width[pin2dmd_frame_buffer_position],
                          dmd_frame_buffer_height[pin2dmd_frame_buffer_position], dmd_planes_buffer,
                          dmd_frame_buffer_depth[pin2dmd_frame_buffer_position]);
    ...
    while (true) {
        ...
        while (zedmd_frame_buffer_position != dmd_frame_buffer_position) {
            ...
            if (opt_serum) {
                ...
                Serum_SetStandardPalette(zedmd.GetDefaultPalette(dmd_frame_buffer_depth[zedmd_frame_buffer_position]),
                                         dmd_frame_buffer_depth[zedmd_frame_buffer_position]);

                if (Serum_ColorizeWithMetadata(renderBuffer, prevWidth, prevHeight, &palette[0], &rotations[0],
                ...
                switch (prevBitDepth) {
                    case 2: zedmd.RenderGray2(renderBuffer); break;

                    case 4: zedmd.RenderGray4(renderBuffer); break;

                    default: printf("Unsupported ZeDMD bit depth: %d\n", prevBitDepth);
                }
            }
...
int CALLBACK
OnAudioAvailable(PinmameAudioInfo* p_audioInfo, const void* p_userData) {
    if (opt_debug) {
...
int CALLBACK
OnAudioUpdated(void* p_buffer, int samples, const void* p_userData) {
...
void CALLBACK
OnSolenoidUpdated(PinmameSolenoidState* p_solenoidState, const void* p_userData) {
    if (opt_debug) {
...
void CALLBACK
OnMechAvailable(int mechNo, PinmameMechInfo* p_mechInfo, const void* p_userData) {
    if (opt_debug) {
...
void CALLBACK
OnMechUpdated(int mechNo, PinmameMechInfo* p_mechInfo, const void* p_userData) {
    if (opt_debug) {
...
void CALLBACK
OnConsoleDataUpdated(void* p_data, int size, const void* p_userData) {
    if (opt_debug) {
...
int CALLBACK
IsKeyPressed(PINMAME_KEYCODE keycode, const void* p_userData) {
    return 0;
}
...
main(int argc, char* argv[]) {
    ...
    while (cag_option_fetch(&cag_context)) {
        ...
        switch (identifier) {
            case 'c': config_file = cag_option_get_value(&cag_context); break;
            case 'r': opt_rom = cag_option_get_value(&cag_context); break;
            case 's': opt_serial = cag_option_get_value(&cag_context); break;
            case 'n': opt_no_serial = true; break;
            case 'u': opt_serum = true; break;
            case 't': opt_serum_timeout = cag_option_get_value(&cag_context); break;
            case 'p': opt_serum_skip_frames = cag_option_get_value(&cag_context); break;
            case 'i': opt_console_display = true; break;
